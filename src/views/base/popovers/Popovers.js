...
const Popovers = () => {
  ...
  const caculateTotalPrice = (products, shippingFee) => {
    let totalPrice = 0
    products.forEach((product) => {
      totalPrice += parseInt(product.price) * parseInt(product.quantity)
    })
    return totalPrice + parseInt(shippingFee)
  }

  const total = caculateTotalPrice(parsedProducts, dataOrder['Giá dịch vụ vận chuyển'])
    ...
    <CRow>
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader>
            <strong>Xem chi tiết đơn hàng:</strong> <small>{OrderID}</small>
          </CCardHeader>
          {dataOrder && (
            <CCardBody>
              <CContainer>
                <CRow>
                  <CCol xs={4}>
                    <div>
                      ...
                      <p className=' mt-2'>Ngày giao hàng</p>
                      <CFormInput id="inputAddress" value={deliveryDate} readOnly />
                    ...
                    <CRow>
                      <CCol xs={6}>
                        ...
                        <CButton color="primary" className=" m-2">
                          Tổng số tiền: {total} vnđ
                        </CButton>
                      </CCol>
                      <CCol xs={6}>
                        <strong className="mt-3">Tài xế</strong>
                        <p className="mt-3 mb-1">Tên tài xế</p>
                      <CFormInput id="inputAddress" value={driverName} readOnly />
                        <p className="mt-3 mb-1">Số điện thoại tài xế</p>
                        <CFormInput id="inputAddress" value={driverPhone} readOnly />

                        <p className="mt-3 mb-1">Phương tiện vận chuyển</p>
                        <CFormInput id="inputAddress" value={Bien_so} readOnly />
                      </CCol>
                    </CRow>
                  </CCol>
                </CRow>
